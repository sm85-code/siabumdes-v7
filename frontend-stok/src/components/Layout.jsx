...
import { LayoutDashboard, Package, ArrowLeftRight, LogOut, Store, ShieldAlert, Menu, X } from "lucide-react";
...
export default function Layout() {
  ...
  return <div className="min-h-screen flex" style={{ background: "var(--bg)" }}>
    <div className="lg:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between px-4 h-14 bg-white border-b" style={{ borderColor: "var(--border)" }}><div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--primary-light)] text-[var(--primary-dark)]"><Store size={17}/></div><span className="font-heading font-bold text-sm">BUMDES Karya Raharja</span></div><button type="button" onClick={() => setOpen(!open)} className="p-2" aria-label="Buka menu">{open ? <X size={22}/> : <Menu size={22}/>}</button></div>
    <aside className={`fixed lg:sticky top-0 left-0 h-[100dvh] w-72 z-50 flex flex-col bg-white border-r transform transition-transform lg:transform-none ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`} style={{ borderColor: "var(--border)" }}>
      <div className="p-6 flex items-center gap-3 border-b" style={{ borderColor: "var(--border)" }}><div className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--primary-light)] text-[var(--primary-dark)]"><Store size={23}/></div><div><div className="font-heading font-bold text-base">BUMDES</div><div className="text-xs text-[var(--text-muted)]">Karya Raharja</div></div></div>
