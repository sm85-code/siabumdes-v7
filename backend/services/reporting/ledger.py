...
async def _ledger_data(account_code: str, start_date: Optional[str], end_date: Optional[str], unit_usaha_id: Optional[str] = None):
    ...
    acc = await db.accounts.find_one({"code": account_code, "group": grp}, {"_id": 0})
    if not acc:
        raise HTTPException(status_code=404, detail=f"Kode akun {account_code} tidak ditemukan di kelompok {grp}")

    base_or = {"$or": [{"debit_account_code": account_code}, {"credit_account_code": account_code}]}
    q: dict = dict(base_or)
    if grp == "BUMDES":
    ...
    if start_date:
        ...
        prev_txs = await db.transactions.find(prev_q, {"_id": 0}).to_list(20000)
        for tx in prev_txs:
