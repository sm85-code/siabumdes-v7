    ...
    for sheet_name in wb.sheetnames:
        ...
        for idx, row in enumerate(rows[1:], start=2):
            ...
            if normal_balance not in ("debit", "kredit"):
                ...
                continue
            # Duplicate check
            if await db.accounts.find_one({"code": code, "group": sheet_name}):
                skipped += 1
                continue
            acc = Account(
...
async def reset_all_accounts(confirm: str = "", _: dict = Depends(require_roles(*ADMIN_LEVEL))):
    ...
    r = await db.accounts.delete_many({})
...
async def create_account(payload: AccountCreate, _: dict = Depends(require_roles(*ADMIN_LEVEL))):
    ...
    if await db.accounts.find_one({"code": payload.code, "group": grp}):
...
async def update_account(code: str, payload: AccountCreate, group: str = "BUMDES",
    ...
    existing = await db.accounts.find_one({"code": code, "group": group}, {"_id": 0})
    if not existing:
    ...
    if (data["code"] != code or target_group != group) and \
            await db.accounts.find_one({"code": data["code"], "group": target_group}):
        raise HTTPException(status_code=400, detail=f"Kode akun {data['code']} sudah ada di kelompok {target_group}")
    await db.accounts.update_one({"code": code, "group": group}, {"$set": data})
    updated = await db.accounts.find_one({"code": data["code"], "group": target_group}, {"_id": 0})
...
async def delete_account(code: str, group: str = "BUMDES",
    ...
    if group == "BUMDES":
        ...
        unit_doc = await db.unit_usaha.find_one({"code": group}, {"_id": 0, "id": 1})
    ...
    r = await db.accounts.delete_one({"code": code, "group": group})
