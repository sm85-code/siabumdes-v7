...
async def close_period(payload: dict, dep: dict = Depends(require_roles(*ADMIN_LEVEL))):
    ...
    # Cek sudah closed?
    if await db.closed_periods.find_one({"period": period, "group": group}):
        raise HTTPException(400, f"Periode {period} ({group}) sudah ditutup")

    # Derive unit_usaha_id from group
    unit_uid = None
    if group != "BUMDES":
        u = await db.unit_usaha.find_one({"code": group}, {"_id": 0, "id": 1})
        if not u:
    ...
    if entries:
        await db.transactions.insert_many(entries)
    await db.closed_periods.insert_one({"period": period, "group": group, "closed_at": now, "closed_by": user.id, "laba_bersih": net})
...
async def list_closed_periods(_: dict = Depends(get_current_user_payload)):
    docs = await db.closed_periods.find({}, {"_id": 0}).sort("period", -1).to_list(500)
    return docs
...
async def reopen_period(period: str, group: str, dep: dict = Depends(require_roles(*ADMIN_LEVEL))):
    ...
    r = await db.transactions.delete_many({"reference": f"CLOSE-{period}-{group}", "is_closing": True})
    await db.closed_periods.delete_one({"period": period, "group": group})
