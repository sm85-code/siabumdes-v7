...
async def public_summary():
    ...
    accounts = {a["code"]: a for a in await db.accounts.find({}, {"_id": 0}).to_list(500)}
    ...
    unit_count = await db.unit_usaha.count_documents({})
    unit_active = await db.transactions.distinct(
        "unit_usaha_id",
...
async def dashboard(
    ...
    if is_pengelola and user.unit_usaha_id:
        ...
        kpi_group = "BUMDES"
    kpi_txs = await db.transactions.find(kpi_q, {"_id": 0}).to_list(20000)

    # Unit summaries: semua unit + BUMDES (untuk breakdown non-pengelola)
    all_txs = await db.transactions.find(date_q, {"_id": 0}).to_list(20000) if not is_pengelola else kpi_txs
    ...
    for doc in await db.accounts.find({}, {"_id": 0}).to_list(500):
