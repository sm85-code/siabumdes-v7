...
async def export_transactions_excel(
    ...
    units = {u["id"]: u for u in await db.unit_usaha.find({}, {"_id": 0}).to_list(50)}
    ...
    if all_data:
        ...
        if payload.get("role") == "pengelola":
            ...
            all_txs = await db.transactions.find({"unit_usaha_id": uid}, {"_id": 0}).sort("date", 1).to_list(50000)
            wb.remove(wb.active)
            ...
            all_txs = await db.transactions.find({}, {"_id": 0}).sort("date", 1).to_list(50000)
        ...
        txs = await db.transactions.find(q, {"_id": 0}).sort("date", 1).to_list(20000)
