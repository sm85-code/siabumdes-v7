...
async def upload_proof(
    ...
    tok = await db.oauth_tokens.find_one({"provider": "gdrive"}, {"_id": 0})
    ...
    if tx.get("unit_usaha_id"):
        u = await db.unit_usaha.find_one({"id": tx["unit_usaha_id"]}, {"_id": 0, "code": 1})
    ...
    max_urut = 0
    async for other in db.transactions.find(q_same, {"_id": 0, "proofs": 1, "proof": 1}):
...
async def verify_proofs(dep: dict = Depends(require_roles("admin", "direktur", "bendahara", "pengelola"))):
    ...
    q: dict = {"$or": [{"proofs": {"$exists": True, "$ne": []}}, {"proof": {"$exists": True}}]}
    ...
    checked = 0
    removed = 0
    async for tx in db.transactions.find(q, {"_id": 0, "id": 1, "proofs": 1, "proof": 1}):
