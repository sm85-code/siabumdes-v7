"""Generic JSONB document repository over PostgreSQL."""
...
from sqlalchemy import CheckConstraint, DateTime, Index, Numeric, String, and_, cast, delete, func, or_, select, text
...
def _matches_filter(value: dict[str, Any], criteria: dict[str, Any]) -> bool:
    for key, expected in criteria.items():
        if key == "any_of":
            if not any(_matches_filter(value, branch) for branch in expected):
                return False
            continue
        if key == "all_of":
            if not all(_matches_filter(value, branch) for branch in expected):
                return False
            continue
        actual = _path(value, key)
        if isinstance(expected, dict) and any(
            op in expected
            for op in ("exists", "in", "not_in", "not_equal", "at_least", "above", "at_most", "below", "gte", "gt", "lte", "lt", "$exists", "$in", "$nin", "$ne", "$gte", "$gt", "$lte", "$lt")
        ):
            for raw_op, operand in expected.items():
                ...
                if op == "in" and actual not in operand:
                    return False
                if op == "not_in" and actual in operand:
                    return False
                if op == "not_equal" and actual == operand:
                    return False
                if op in {"at_least", "above", "at_most", "below"}:
                    ...
                    try:
                        ...
                        if op == "at_least" and left < right:
                            return False
                        if op == "above" and left <= right:
                            return False
                        if op == "at_most" and left > right:
                            return False
                        if op == "below" and left >= right:
                            return False
...
def _filter_condition(criteria: dict[str, Any]):
    ...
    for key, expected in criteria.items():
        if key in {"any_of", "$or"}:
            conditions.append(or_(*(_filter_condition(branch) for branch in expected)))
            continue
        if key in {"all_of", "$and"}:
            conditions.append(and_(*(_filter_condition(branch) for branch in expected)))
            continue
        ...
        if isinstance(expected, dict):
            for raw_op, value in expected.items():
                op = {"$exists": "exists", "$in": "in", "$nin": "not_in", "$ne": "not_equal", "$gte": "at_least", "$gt": "above", "$lte": "at_most", "$lt": "below"}.get(raw_op, raw_op)
                if op == "exists":
                    conditions.append(StoredEntity.payload[key].is_not(None) if value else StoredEntity.payload[key].is_(None))
                elif op == "in":
                    conditions.append(column.in_([str(item) for item in value]))
                elif op == "not_in":
                    conditions.append(column.not_in([str(item) for item in value]))
                elif op == "not_equal":
...
def _project(payload: dict[str, Any], projection: dict[str, int] | None) -> dict[str, Any]:
    ...
    if included:
        return {key: _path(payload, key) for key in included if _path(payload, key) is not None}
...
class Query:
    def __init__(self, loader):
        self._loader, self._ops = loader, []

    def order(self, key: str, direction: int):
        self._ops.append(("order", key, direction))
        return self

    def offset(self, amount: int):
        self._ops.append(("offset", amount))
        return self

    def limit(self, amount: int):
        self._ops.append(("limit", amount))
        return self

    async def all(self, length: int = 0):
        return await self._loader(self._ops, length)
...
class Repository:
    ...
    def select(self, criteria=None, projection=None):
        async def loader(ops, length):
            ...
            for op in ops:
                if op[0] == "order":
                    statement = statement.order_by(_json_value(op[1]).desc() if op[2] < 0 else _json_value(op[1]).asc())
                elif op[0] == "offset":
                    statement = statement.offset(op[1])
                elif op[0] == "limit":
                    statement = statement.limit(op[1])
            if length:
    ...
    async def select_one(self, criteria=None, projection=None):
        rows = await self.select(criteria, projection).all(1)
        return rows[0] if rows else None

    async def count(self, criteria=None):
        async with SessionLocal() as session:
            return int(
                (await session.scalar(select(func.count()).select_from(StoredEntity).where(StoredEntity.namespace == self.namespace, _filter_condition(criteria or {})))) or 0
            )
    ...
    async def create(self, payload):
        item = jsonable_encoder(copy.deepcopy(payload))
        item_id = str(item.get("id"))
        now = datetime.now(timezone.utc)
        async with SessionLocal.begin() as session:
            session.add(StoredEntity(namespace=self.namespace, id=item_id, payload=item, created_at=now, updated_at=now))
    ...
    async def create_many(self, payloads):
        ...
        async with SessionLocal.begin() as session:
            session.add_all(
                [
                    StoredEntity(namespace=self.namespace, id=str(item.get("id")), payload=jsonable_encoder(copy.deepcopy(item)), created_at=now, updated_at=now)
                    for item in payloads
                ]
            )
        return OperationResult()

    async def modify_one(self, criteria, changes, upsert=False):
        return await self._modify(criteria, changes, upsert, True)

    async def modify_many(self, criteria, changes, upsert=False):
        return await self._modify(criteria, changes, upsert, False)
    ...
    async def _modify(self, criteria, changes, upsert, one):
        async with SessionLocal.begin() as session:
            ...
            if not rows and upsert:
                ...
                data.update(changes.get("set", changes))
                data.setdefault("id", os.urandom(12).hex())
                data = jsonable_encoder(data)
                now = datetime.now(timezone.utc)
                session.add(StoredEntity(namespace=self.namespace, id=str(data["id"]), payload=data, created_at=now, updated_at=now))
                return OperationResult(inserted_id=str(data["id"]))
            for row in rows:
                data = copy.deepcopy(row.payload)
                data.update(changes.get("set", {}))
                for key in changes.get("unset", {}):
                    data.pop(key, None)
    ...
    async def remove_one(self, criteria):
        return await self._remove(criteria, True)

    async def remove_many(self, criteria):
        return await self._remove(criteria, False)
